...
export async function scheduleFollowUps(flow:WorkflowService,now=new Date()):Promise<void>{
  ...
  const cases=await flow.db.botApodExpediente.findMany({where:{automationPaused:false,optOutAt:null,nextReminderAt:{lte:now},currentState:{in:[...FOLLOW_UP_STATES]}},take:100,orderBy:{nextReminderAt:'asc'}});
  for(const row of cases){
    ...
    await flow.locked(row.id,async signal=>{
      ...
      if(signal.aborted||c.automationPaused||c.optOutAt||!c.reminderAnchorAt||!canFollowUp(c))return;
      ...
      if(days>=30){
        await flow.db.$transaction(async tx=>{
          ...
          await tx.botApodHumanTask.upsert({where:{dedupeKey:`expiry:${c.id}:${c.reminderCycle}`},create:{expedienteId:c.id,dedupeKey:`expiry:${c.id}:${c.reminderCycle}`,kind:'NO_RESPONSE_30_DAYS',assignedTo:'MANAGEMENT',reason:'30 días sin respuesta. Revisar el expediente y las condiciones contractuales antes de decidir cualquier cargo.',evidence:json({stepReached:c.stepReached,anchor:c.reminderAnchorAt,reminderCount:c.reminderCount,chargesApplied:false})},update:{}});
        });return;
      }
...
export async function pollAviso27(flow:WorkflowService,adapters:ConfiguredAdapters,signal?:AbortSignal):Promise<void>{
  ...
  for(const item of page.items){
    ...
    const trigger=await flow.db.botApodTrigger.upsert({where:{externalId:item.externalId},create:{externalId:item.externalId,projectId:item.projectId,evidenceRef:item.evidenceRef},update:{}});
    if(trigger.status==='PROCESSED')continue;
    try{
      const candidate=await km.resolveTriggerExpediente(item.projectId);
      ...
      if(signal?.aborted)throw new Error('AVISO27_LOCK_LOST');
      await flow.linkKmaleon(candidate,{source:'AVISO_27',triggerId:trigger.id});
