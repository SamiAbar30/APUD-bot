...
import { EventOrigin, EventType, originOf, type WorkflowEvent } from '../domain/fsm/states.js';
...
export class WorkflowService {
  ...
  async transition(tx:Prisma.TransactionClient,c:BotApodExpediente,type:string,payload:Record<string,unknown>={},patch:WorkflowPatch={},operator='SYSTEM_BOT') {
    ...
    // A client who just wrote or tapped is always told a person takes over; a handover in silence
    // looks like the bot vanished (live test 24 Sep 15:13: consent tap escalated with no word).
    const clientWaiting=originOf(type)===EventOrigin.CLIENT&&c.currentState!==ApodState.ESCALATED_HUMAN;
    if(decision.actionPayload.kind==='ESCALATE_HUMAN'&&type!==EventType.CLIENT_OPT_OUT&&!c.optOutAt&&(decision.actionPayload.clientNoticeTemplate||clientWaiting))await tx.botApodAccion.create({data:{expedienteId:c.id,decisionId:randomUUID(),expectedVersion:nextVersion,actionType:'SEND_WHATSAPP_MESSAGE',payload:json({template:payload.requiresHumanReview===true?'CONVERSATION_REPLY':'HUMAN_HANDOFF_NOTICE',...(payload.requiresHumanReview===true?{variables:{replyText:payload.responseText},contextStep:c.stepReached}:{}),parentDecisionId:decision.decisionId,humanHandoff:true,handoffReason:payload.handoffReason??'HUMANO'}),idempotencyKey:`handoff-${decision.decisionId}`}});
