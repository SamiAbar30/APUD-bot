...
const templates:Record<TemplateId,Renderer>={
  ...
  [TemplateId.MOBILE_EXPORT_GUIDE]:()=>({text:'Abre la aplicación del certificado y busca «copia de seguridad» o «exportar». Protege la copia con una contraseña y consérvala para instalarla en el ordenador. Si prefieres asistencia del despacho, el equipo te indicará el canal seguro; no envíes la contraseña por este chat.',buttons:[{id:'DEVICE_PC',title:'Ya está en el PC'},{id:'NEEDS_ASSISTANCE',title:'Necesito asistencia'}]}),
  ...
  [TemplateId.ASSIST_CONSENT_REQUEST]:()=>({text:'Podemos organizar la asistencia del despacho con tu certificado. El equipo te explicará el alcance y cómo aportar el archivo y su contraseña por el canal seguro. Esta conversación no autoriza por sí sola la firma. ¿Quieres continuar con la asistencia?',buttons:[{id:'CONSENT_YES',title:'Quiero asistencia'},{id:'CONSENT_NO',title:'Prefiero que no'}]}),
  [TemplateId.ASSIST_SEND_CERT_INSTRUCTIONS]:()=>({text:'Para continuar, conserva la copia de tu certificado (.p12 o .pfx) y su contraseña. El equipo te indicará el canal seguro para aportarlos y comprobará que la copia funciona; no los envíes por este chat.',buttons:[{id:'CONSENT_NO',title:'Mejor no'},help]}),
  ...
  [TemplateId.PC_TUTORIAL]:()=>({text:`Te paso la guía con los datos de los profesionales y las facultades que debes revisar.\n\nDesde el ordenador donde tienes el certificado, comprueba AutoFirma (https://firmaelectronica.gob.es/Home/Descargas.htm) y entra en ${officialLinks.sede} por «Certificado Digital». Sigue la guía y revisa los datos antes de firmar. Cuando termines, indícanoslo por aquí. Si necesitas asistencia del despacho, el equipo te indicará el canal seguro para el certificado.`,attachment:'TUTORIAL'}),
  ...
  [TemplateId.CERTIFICATE_COPY_HELP]:(c,_version,variables)=>{
    ...
    return {text:'Si no aparece, revisa la opción de copia de seguridad o exportación y protege la copia con una contraseña. Conserva ambos; para recibir asistencia, el equipo te indicará cómo aportarlos por el canal seguro.'};
  },
