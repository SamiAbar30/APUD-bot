import { KmaleonIdentitySchema,KmaleonAnnotationSchema,KmaleonProjectAddressSchema,KmaleonExpedienteCandidateSchema,type KmaleonVerifiedAddress,type KmaleonResponseMapping,type KmaleonAnnotation,type KmaleonExpedienteCandidate,type DocumentProof,type NoticeProof } from '../../contracts/kmaleon.contract.js';
import { AdapterError,requireId,requireValue,sha256 } from '../common/http.js';
import { KmaleonClient } from './kmaleon-client.js';
export interface KmaleonGatewayOptions {recipientCode?:number;mapping:KmaleonResponseMapping;maxPages?:number;readbackDelayMs?:number}
...
export class KmaleonGateway {
  ...
  private requiredRecipientCode():number{
    ...
    if(typeof code!=='number'||!Number.isSafeInteger(code)||code<1)throw new AdapterError('KMALEON_RECIPIENT_NOT_CONFIGURED');
    return code;
  }
  private async checkIdentity(projectId:string,expectedDni:string):Promise<void>{
    ...
    const parsed=KmaleonIdentitySchema.safeParse(this.options.mapping.projectIdentity(raw));
  ...
  private async annotations(projectId:string):Promise<KmaleonAnnotation[]>{
    ...
    for(let page=0;page<(this.options.maxPages??100);page++){
      const raw=await this.client.invokeRead('calendar/annotations/getAnnotations',{project_id:Number(projectId)||projectId,pageNum:page,filter:''});
  ...
  private async proveDocument(row:KmaleonAnnotation,projectId:string,hash:string,key:string):Promise<DocumentProof>{
    if(!row.documentId)throw new AdapterError('KMALEON_DOCUMENT_REFERENCE_MISSING','uncertain');
    ...
    return {verified:true,projectId,annotationId:row.id,documentId:row.documentId,sha256:hash,idempotencyKey:key};
  }
  private annotation(text:string,pending:boolean):Record<string,unknown>{
    ...
    return {fecha:`${part('day')}/${part('month')}/${part('year')}`,hora:`${part('hour')}:${part('minute')}`,duracion:'00:00',tipo:{codigo:pending?'R':'H',descripcion:pending?'Recordatorio':'Historial'},opciones:pending?'PIE':'E',destinatario:{codigo:recipientCode},eshilopadre:0,eshilohijo:0,eshiloactivo:0,codigopadre:0,texto:text,comentario:text};
  }
  async uploadAndVerifyDocument(input:DocumentInput):Promise<DocumentProof>{
    this.requiredRecipientCode();
    ...
    const recipientCode=this.requiredRecipientCode();
    if(prior.some(row=>row.text.includes(`[APOD:${input.idempotencyKey}:`)&&(row.text!==text||row.recipientCode!==recipientCode)))throw new AdapterError('KMALEON_IDEMPOTENCY_REFERENCE_CONFLICT','uncertain');
    const existing=this.exact(prior,text);
    if(existing)return this.proveDocument(existing,input.projectId,hash,input.idempotencyKey);
    ...
    try { await this.client.invokeWrite('calendar/annotations/newAnnotation',{project_id:Number(input.projectId)||input.projectId,annotation:{...this.annotation(text,false),documento:{file:input.fileName,content:input.buffer.toString('base64')}}}); }
    catch(error){if(error instanceof AdapterError&&error.outcome==='not_applied')throw error;}
    for(let attempt=0;attempt<3;attempt++){
      try{await this.checkIdentity(input.projectId,input.expectedDni);const row=this.exact(await this.annotations(input.projectId),text);if(row)return await this.proveDocument(row,input.projectId,hash,input.idempotencyKey);}catch(error){if(error instanceof AdapterError&&['KMALEON_DOCUMENT_HASH_MISMATCH','KMALEON_DUPLICATE_REFERENCE','KMALEON_PROJECT_IDENTITY_MISMATCH'].includes(error.code))throw error;}
  ...
  async notifyCarmen(input:{projectId:string;expectedDni:string;idempotencyKey:string;documentProof:DocumentProof;isProvisional:boolean;reconcileOnly?:boolean}):Promise<NoticeProof>{
    const recipientCode=this.requiredRecipientCode();
    requireId(input.idempotencyKey,'IDEMPOTENCY_KEY');const proof=input.documentProof;
    if(!proof.verified||proof.projectId!==input.projectId||!/^[a-f0-9]{64}$/.test(proof.sha256))throw new AdapterError('INVALID_DOCUMENT_PROOF');
    await this.checkIdentity(input.projectId,input.expectedDni);
    ...
    if(!documentRow||documentRow.text!==documentText||documentRow.recipientCode!==recipientCode)throw new AdapterError('KMALEON_DOCUMENT_PROOF_STALE');
    await this.proveDocument(documentRow,input.projectId,proof.sha256,proof.idempotencyKey);
    const text=`CARMEN: APODERAMIENTO ${input.isProvisional?'PROVISIONAL; PENDIENTE SUBSANACION':'VALIDADO'}. DOCUMENTO ${proof.documentId} [APOD-NOTICE:${input.idempotencyKey}:${proof.sha256}]`;
    ...
    const existing=this.exact(before,text);if(existing){if(!existing.pending)throw new AdapterError('KMALEON_NOTICE_NO_LONGER_PENDING','uncertain');return result(existing);}
