...
export interface WhatsAppOptions {accessToken:string;phoneNumberId:string;apiVersion:string;writesEnabled?:boolean;allowedRecipients?:readonly string[];mediaHosts?:string[];maxMediaBytes?:number;timeoutMs?:number}
...
export class WhatsAppClient {
  ...
  private endpoint(id:string):URL {if(!/^\d+$/.test(id))throw new AdapterError('INVALID_META_ID');return new URL(`/${this.options.apiVersion}/${id}`,this.origin);}
  ...
  private async send(to:string,body:Record<string,unknown>):Promise<WhatsAppAccepted>{
    ...
    const url=this.endpoint(this.options.phoneNumberId);url.pathname+='/messages';
  ...
  sendDocumentButtons(to:string,mediaId:string,body:string,buttons:Array<{id:ReplyButtonId;title:string}>):Promise<WhatsAppAccepted>{
    if(!/^\d+$/.test(mediaId)||!body||body.length>1024||buttons.length<1||buttons.length>3||buttons.some(b=>!b.title||b.title.length>20)||new Set(buttons.map(b=>b.id)).size!==buttons.length)throw new AdapterError('INVALID_DOCUMENT_BUTTONS');
  ...
  sendDocument(to:string,mediaId:string,filename?:string,caption?:string):Promise<WhatsAppAccepted>{if(!/^\d+$/.test(mediaId))throw new AdapterError('INVALID_MEDIA_ID');return this.send(to,{type:'document',document:{id:mediaId,...(filename?{filename:filename.slice(0,200)}:{}),...(caption?{caption:caption.slice(0,1024)}:{})}});}
