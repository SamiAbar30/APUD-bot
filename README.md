...
- Demo WhatsApp: `DEMO_DATA_ENABLED=true` y `DEMO_WHATSAPP_RECIPIENTS=34600000000` habilitan el expediente local de prueba. `npm run demo:seed` sólo escribe el expediente demo; `npm run demo:start` inicia el evento `CASE_OPENED` a través de la API local. Los envíos posteriores son reales y se bloquean para cualquier otro destinatario.
