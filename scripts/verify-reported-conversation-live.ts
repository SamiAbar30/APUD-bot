...
try{
  const original=await db.botApodExpediente.findUniqueOrThrow({where:{telefono:demoPhoneNumber()}});
  assert.equal(original.kmaleonExpedienteId,`demo-kmaleon-${demoPhoneNumber()}`);assert.equal(original.optOutAt,null);
